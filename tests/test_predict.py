...
def mock_config(tmp_path):
    ...
    with open(config_path, "w") as f:
        json.dump(config, f)  # или yaml.dump
    return str(config_path)


@pytest.fixture
def mock_gt(tmp_path):
    """Создаёт временный train_gt.json."""
    gt = {"img1.jpg": 0}
...
def test_postprocess_logits_to_class_id(mock_config, monkeypatch):
    ...
    preds = classify(
        model_path="any",
        test_img_dir="any",
        config_path=mock_config,    # ← реальный путь к config
        batch_size=1
    )

    assert preds == {"img1.jpg": 0}  # argmax([2.1, -0.3, 1.8]) = 0
...
def test_classify_empty_test_directory(monkeypatch, mock_config):
    """Проверка: пустая папка → пустой результат."""
    class EmptyDataset:
...
def test_compute_accuracy_empty_gt(mock_gt, tmp_path):
    """Проверка: пустой GT → accuracy = None."""
    empty_gt = {}
...
def test_classify_nonexistent_test_dir(monkeypatch, mock_config):
    """Проверка: несуществующая папка → ошибка в InferenceDataset."""
    monkeypatch.setattr("predict.BirdModel", MockBirdModel)
    # Не подменяем InferenceDataset → он попытается прочитать реальную папку
